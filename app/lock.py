...
# سناریو 1: Race Condition - اجرای همزمان نخ ها و دسترسی به یک متغیر مشترک بدون استفاده از قفل(بدون همگام سازی)
def lock_scenario1():
    counter = 0
    output = []
    ...
    class CounterThread(Thread):
        ...
        def run(self):
            ...
            counter = temp + 1
            output.append(
                f"---> {self.name} write counter = {counter}"
            )
    ...
    explanation.append(f"❌ مقدار مورد انتظار: 10 (چون 10 نخ هر کدام 1 بار افزایش دادند)")
    if counter < 10:
        explanation.append(f"⚠️ تعداد افزایش‌های از دست رفته: {10 - counter}")
        explanation.append("🔴 دلیل: Race Condition - چند نخ همزمان مقدار را خواندند و تداخل ایجاد شد")
    else:
    ...
    explanation.append("اگر دو نخ همزمان مقدار را بخوانند، هر دو مقدار یکسان را می‌بینند و پس از افزایش، مقدار نهایی فقط یک بار افزایش می‌یابد.")
    explanation.append("\n📚 مثال:")
    explanation.append("نخ A: مقدار 5 را می‌خواند")
    explanation.append("نخ B: مقدار 5 را می‌خواند (قبل از اینکه نخ A مقدار نهایی را بنویسد)")
...
# سناریو 2: همگام سازی نخ ها با استفاده از قفل  برای دسترسی به یک متغیر مشترک
def lock_scenario2():
    ...
    explanation.append("قفل (Lock) از تداخل نخ‌ها جلوگیری می‌کند و صحت داده را تضمین می‌نماید،")
...
def lock_scenario3():
    ...
    explanation.append("چندین کاربر همزمان به یک حساب بانکی متصل می‌شوند و تراکنش انجام می‌دهند.")
    ...
    explanation.append("2️⃣ در طول بررسی و انجام تراکنش، هیچ تراکنش دیگری نمی‌تواند موجودی را تغییر دهد")
    explanation.append("3️⃣ بعد از اتمام تراکنش، قفل آزاد می‌شود تا تراکنش بعدی انجام شود")
    explanation.append("")
    ...
    explanation.append("")
    explanation.append("💡 نتیجه‌گیری :")
    explanation.append("این سناریو نشان می‌دهد که چگونه قفل (Lock) در سیستم‌های واقعی مانند بانک‌ها، صف‌های خرید، رزرو بلیط، و انبارداری استفاده می‌شود.")
    explanation.append("قفل تضمین می‌کند که عملیات همزمان روی داده‌های مشترک، صحیح و بدون تداخل انجام شوند.")
...
LOCK_SCENARIOS = {1: lock_scenario1, 2: lock_scenario2, 3: lock_scenario3}
def run_lock_scenario(scenario_id: int):
    if scenario_id not in LOCK_SCENARIOS:
        raise ValueError(f"سناریوی {scenario_id} وجود ندارد. سناریوهای موجود: 1, 2, 3")

    return LOCK_SCENARIOS[scenario_id]()
